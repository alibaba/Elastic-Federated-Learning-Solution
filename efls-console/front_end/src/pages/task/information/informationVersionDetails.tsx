...
import { getInformationTaskIntraInfo, getInformationTaskInterInfo, getInformationTaskInstanceList, informationTaskInstanceStatus, instanceInformationTaskRun, instanceInformationTaskUpdate } from './service';
import { informationTaskInstanceTableColumns } from './utils';
...
const versionDetails: React.FC<versionDetailsProps> = (props) => {
  ...
  const { loading, run: informationTaskIntraInfo } = useRequest(getInformationTaskIntraInfo, {
    manual: true,
  ...
  const { loading: intertLoading, run: informationTaskInterInfo } = useRequest(getInformationTaskInterInfo, {
    manual: true,
    onSuccess: (result: any, params) => {
      const { data, rsp_code } = result;
      if (rsp_code == 0 && Object.keys(data).length > 0) {
  ...
  useEffect(() => {
    if (configVisible) {
      if (!versionInterInfo) {
        informationTaskInterInfo(versionInfo.task_inter_id)
      };
    };
  }, [configVisible])
  ...
  const renderTaskInstanceList = async (params: any) => {
    ...
    if (task_inter_id) {
      const { data: { task_instance_list = [], total }, rsp_code } = await getInformationTaskInstanceList({ task_inter_id, ...params });
      return {
  ...
  const handleInstanceRun = (params: any) => {
    informationTaskInstanceStatus(params).then((res: any) => {
  ...
  const handleInstanceUpdate = (id: any) => {
    instanceInformationTaskUpdate({ id, "need_sync": true }).then((res: any) => {
      const { rsp_code, message: info } = res;
      if (rsp_code === 0) {
  ...
  const versionStart = () => {
    instanceInformationTaskRun(versionInfo.task_inter_id).then((res: any) => {
      const { rsp_code, message: info } = res;
      if (rsp_code === 0) {
  ...
  const columns = informationTaskInstanceTableColumns(handleInstanceRun, handleInstanceUpdate, projectConfig.id);
    ...
    <>
      {
        ...
        <ProDescriptions column={2} title={<h3 className={styles.title_before}>版本详情</h3>} >

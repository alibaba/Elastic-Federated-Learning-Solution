...
class ClientSortJoinFunc_local(object):
    ...
    def data_join_client_bucket_file(self, bucket_dir_path):
        ...
        while cur < len(keys_to_join):
            ...
            cur = end
            for i in res_ids:
                self._output_bucket_file.write(self._sample_store.get(i))
...
class data_join_pipeline_local(object):
    def __init__(self,
                 input_path: str,
                 output_path: str,
                 job_name: str,
    ...
    def read_buffer_data_to_bucket(self, thread_id):
        for raw_record in self._data_to_bucket_threads_data_buffer[thread_id]:
            ...
            self._data_to_bucket_file_list_sum[thread_id][bucket_id].write(raw_record.numpy())
    ...
    def read_tf_record_data_to_bucket(self):
        ...
        if not tf.io.gfile.exists(self._bucket_path):
            tf.io.gfile.makedirs(self._bucket_path)
        for worker in range(self._data_to_bucket_threads_num):
            ...
            for i in range(self._bucket_num):
                ...
                if not tf.io.gfile.exists(bucket_dir_path):
                    tf.io.gfile.makedirs(bucket_dir_path)
                bucket_file_path = os.path.join(bucket_dir_path, self._data_to_bucket_file_name.format(i, worker))
                bucket_file = tf.io.TFRecordWriter(bucket_file_path)
        ...
        now_thread = None
        for now_path, subfolder, files in tf.io.gfile.walk(self._input_path):
            for _file in files:
                filename = os.path.join(now_path, _file)
                filename = [filename]
                raw_dataset_tmp = tf.data.TFRecordDataset(filename)
                raw_dataset = []
    ...
    def worker_for_data_join_bucket(self, bucket_id):
        ...
        if not tf.io.gfile.exists(output_bucket_dir_path):
            tf.io.gfile.makedirs(output_bucket_dir_path)
        output_bucket_file_path = os.path.join(output_bucket_dir_path, str(bucket_id) + '.tfrecords')
        output_bucket_file = tf.io.TFRecordWriter(output_bucket_file_path)
    ...
    def data_join_client_workers(self):
        ...
        tf.gfile.DeleteRecursively(self._bucket_path)

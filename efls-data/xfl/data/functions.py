...
class ClientBatchJoinFunc(ClientJoinFunc):
  ...
  def process_element(self, value, ctx: 'ProcessFunction.Context'):
    ...
    self._request_buf[id_key] = value[2]
    if len(self._request_buf) >= self._batch_size:
      request_ids = list(self._request_buf.keys())
      existence = self.client.sync_join(request_ids)
      res_ids = utils.gather_res(request_ids, existence=existence)
      self.check_sum.add_list(res_ids)
      for i in res_ids:
        yield str(self._subtask_index), self._request_buf.get(i)
      log.info("client sync join current idx: {}".format(self.cnt))
      self._request_buf.clear()
    ...
    if timestamp >= s + self._delay:
      ...
      if self._request_buf:
        ...
        for i in res_ids:
          yield str(self._subtask_index), self._request_buf.get(i)
      self._request_buf.clear()
      res = self.client.finish_join(self.check_sum.get_check_sum())
      self._request_buf.clear()
      if not res:
        raise ValueError("Join finish error")
      log.info("End join, checkSum:{}".format(self.check_sum.get_check_sum()))

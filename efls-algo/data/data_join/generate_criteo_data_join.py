...
parser.add_argument('-r', '--follower_drop_ratio', type=float, default=0.9)
...
class BuildTfRecordTask(object):
  ...
  def generate_args(self):
    args = []
    for part_size, output_dir in self._params:
      for i in range(math.ceil(self._labels.shape[0] / float(part_size))):
        end = min(self._labels.shape[0], (i + 1) * part_size)
        args += [[i * part_size, end, i, output_dir]]
    return args
  ...
  def build_follower_tfrecord(self, file_path, start, end, dense, file_num):
    ...
    real_count = 0
    for i in range(start, end):
      # drop some samples random for data-join demo
      if random.random() > _CMD_ARGS.follower_drop_ratio:
        continue
      real_count += 1
      sample_id = np.array([i])
      ...
      writer.write(example.SerializeToString())
    writer.close()
    logging.info('build follower tfrecord success: file_path [%s] count [%d] file_num [%d]' % (file_path, real_count, file_num))

  def __call__(self, start, end, file_no, output_dir):
    label = self._labels
    categorical = self._categorical_features
    dense = self._dense_features
...
def data_preprocess(data_path, block_rows):
  def _data_count():
...
if __name__ == '__main__':
  ...
  data = data_preprocess(_CMD_ARGS.data_path, block_rows)
  labels, categorical_features, dense_features = column_sample(data, block_rows)
  del data

  BuildTfRecordTask._labels = labels
  BuildTfRecordTask._categorical_features = categorical_features
  BuildTfRecordTask._dense_features = dense_features

  params = [[_CMD_ARGS.file_part_size, _CMD_ARGS.output_dir + '/%s']]
